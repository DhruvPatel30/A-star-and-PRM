...
class Patch:
    ...
    def lit_one_obstacle_and_find_nearPatches(self):
        ...
        threading.Timer(10, self.lit_one_obstacle_and_find_nearPatches).start()
...
def main(win, width):                                      # Main loop
    ...
    while time.time() < t_end:
        ...
        start = r_x, r_y, r_h
        try:
            goal = patch.burnt_track[0]
        except:
            continue
        t_start = time.time()
        AStar(grid, robot, start, goal, patch)     
        t_end_ = time.time()
        patch.total_time += t_end      
